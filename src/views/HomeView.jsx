...
const HomeView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await musicApi.getChart();
        setChartData(data);
      } catch (error) {
        console.error("Failed to fetch chart:", error);
        // Fallback to static data will be used
      } finally {
        setLoading(false);
      }
    };
  ...
  const popularPlaylistData =
    chartData?.tracks?.slice(4, 8).map(mapTrackToSlide) || [];

  const newReleasesData2 =
    chartData?.tracks?.slice(8, 10).map(mapTrackToSlide) || [];

  const mixesInspiredByData =
  ...
  const playlistsData = chartData?.artists
    ? [
        {
          category: "daily",
